...
template <typename Dtype>
class EuclideanWithValidLabelLossLayer : public LossLayer<Dtype> {
 public:
  explicit EuclideanWithValidLabelLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "EuclideanWithValidLabelLoss"; }
  /**
   * Unlike most loss layers, in the EuclideanWithValidLabelLossLayer we can backpropagate
   * to both inputs -- override to return true and always allow force_backward.
   */
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return true;
 ...
 protected:
  /// @copydoc EuclideanWithValidLabelLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
...
// added by rgirdhar for use with contrastive loss in between
template <typename Dtype>
class SigmoidCrossEntropyWithValidLabelLossLayer : public LossLayer<Dtype> {
 public:
  explicit SigmoidCrossEntropyWithValidLabelLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param),
          sigmoid_layer_(new SigmoidLayer<Dtype>(param)),
          sigmoid_output_(new Blob<Dtype>()) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "SigmoidCrossEntropyWithValidLabelLoss"; }
  virtual inline int ExactNumBottomBlobs() const { return 3; } 
...
// Forward declare MultiSoftmaxLayer for use in MultiSoftmaxWithLossLayer.
